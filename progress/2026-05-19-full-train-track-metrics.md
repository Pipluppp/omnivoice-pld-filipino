...
The first full evaluation used [omnivoice_evaluation_metrics.py](../notebooks/omnivoice_evaluation_metrics.py) for the base model and the initial 1000-step checkpoint. Later runs used [omnivoice_evaluation_metrics_finetunes.py](../notebooks/omnivoice_evaluation_metrics_finetunes.py) to evaluate the 2000-step checkpoint and the best development-loss checkpoint while reusing the base model metrics from the first full run.
...
| Fine-tuned best | `omnivoice-filipino-full-checkpoint-best` | `1e-5` | best eval loss at step 4900 | 18.52 | -4.03 | 0.604 | 3.61 | Best overall checkpoint by WER, with SIM-o preserved above base and only a small UTMOS drop. |

Current best model: `omnivoice-filipino-full-checkpoint-best`.

Reason: it gives the largest intelligibility improvement, reducing WER from 22.55% to 18.52% on the full test set. That is a 4.03 absolute point reduction, about 17.9% relative WER reduction. It does not win on SIM-o or UTMOS individually, but SIM-o remains slightly above the base model and UTMOS is close to the other fine-tuned checkpoints. For this study, the best checkpoint should be selected by the coherent overall metric pattern, not by a single auxiliary metric.
...
The best checkpoint mainly improves WER by reducing insertions and substitutions compared with the base model. The 2000-step `5e-6` run reduced deletions and substitutions but increased insertions enough to make its total WER slightly worse than base.
...
And so had to test our a new model checkpoint saving to check every few eval steps, then save the model checkpoint only when it's better. So that for these long training runs we'll have access to the best eval finetuned model?

Left the following running:

- [lr=1e-5, steps=5000](https://www.kaggle.com/code/togepiiiiiiiiiiiii/omnivoice-pld-full-train-tokenized-dataset-simple?scriptVersionId=320677078) which uses the new model checkpoint saving only on the best eval loss
