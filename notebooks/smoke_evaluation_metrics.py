...
# # OmniVoice Filipino PLD smoke evaluation on Kaggle
# This notebook-style script evaluates the base and fine-tuned OmniVoice models on
# a 100-sample slice of the PLD Filipino test split. It downloads the fine-tuned
# checkpoint from a W&B artifact, generates speech, computes OmniVoice objective
# metrics, and logs the smoke-test outputs back to W&B.
# The goal is to compare the pretrained OmniVoice checkpoint against our Filipino
# fine-tuned checkpoint under the same zero-shot voice-cloning setup. Each test
# item supplies the target text to synthesize, while a different utterance from
# the same PLD test speaker is used as the voice prompt.
...
# %% [markdown] {"jupyter":{"outputs_hidden":false}}
# ## 1. Settings
# These settings define the Kaggle inputs, model sources, output directories,
# generation parameters, metric batch sizes, and W&B tracking configuration.
# This smoke notebook is meant to validate the full evaluation path before
# committing to the multi-hour full test-set run.

# %% [code] {"jupyter":{"outputs_hidden":false}}
RUN_NAME = "smoke-filipino-pld-test-eval-100"
...
WORK_DIR = Path("/kaggle/working/omnivoice_filipino_pld_eval_smoke")
TEMP_DIR = Path("/kaggle/temp/omnivoice_filipino_pld_eval_smoke")
MANIFEST_DIR = WORK_DIR / "manifests"
RESULTS_DIR = WORK_DIR / "results" / "smoke_test"
BASE_RESULTS_DIR = RESULTS_DIR / "base"
FINETUNED_RESULTS_DIR = RESULTS_DIR / "finetuned"
METRICS_DIR = WORK_DIR / "metrics" / "smoke_test"
CHECKPOINT_DOWNLOAD_DIR = WORK_DIR / "wandb_checkpoint_artifact"
...
EVAL_MAX_SAMPLES: int | None = 100

USE_WANDB = True
WANDB_ENTITY = "duncanb013-polytechnic-university-of-the-philippines"
WANDB_PROJECT = "OmniVoice-PLD-Filipino"
WANDB_MODE = "online"
WANDB_TAGS = ["omnivoice", "filipino", "pld", "kaggle", "evaluation", "smoke"]
WANDB_EVAL_RUN_ID = os.environ.get("WANDB_EVAL_RUN_ID") or (
...
# and the script will skip W&B artifact download.
FINETUNED_CHECKPOINT_LOCAL_PATH: str | None = None
...
# `WANDB_API_KEY`, allowing this notebook to pull the fine-tuned checkpoint
# artifact and log the evaluation run without manually uploading a Kaggle Model.
...
# We install OmniVoice in an isolated `uv` environment and add only the eval
# dependencies needed for this run. The metrics use the OmniVoice evaluation
# modules directly, which keeps the implementation aligned with the upstream
# paper/code instead of reimplementing the scoring logic from scratch.
...
# %% [markdown] {"jupyter":{"outputs_hidden":false}}
# ## 4. Start W&B run and download fine-tuned checkpoint
# This creates a separate W&B evaluation run. The fine-tuned checkpoint is pulled
# with `run.use_artifact(...)`, so W&B records the checkpoint as an input to this
# evaluation. This avoids the W&B web UI zip download limit and keeps the
# evaluation reproducible from the artifact name and alias.
# The downloaded artifact must contain a complete Hugging Face-style checkpoint
# folder, not just `model.safetensors`; OmniVoice also needs `config.json` and
# tokenizer files when loading the fine-tuned model for inference.
...
if USE_WANDB and WANDB_MODE != "disabled":
    wandb_kwargs = {
        ...
        "config": {
            ...
            "finetuned_wandb_artifact": FINETUNED_WANDB_ARTIFACT,
...
if FINETUNED_CHECKPOINT_LOCAL_PATH:
    FINETUNED_CHECKPOINT = find_checkpoint_dir(Path(FINETUNED_CHECKPOINT_LOCAL_PATH))
else:
    if wandb_run is None:
        raise RuntimeError(
            "W&B is disabled but FINETUNED_CHECKPOINT_LOCAL_PATH is not set."
        )
    print("Downloading W&B artifact:", FINETUNED_WANDB_ARTIFACT)
    artifact = wandb_run.use_artifact(FINETUNED_WANDB_ARTIFACT, type="model")
    downloaded_root = Path(artifact.download(root=str(CHECKPOINT_DOWNLOAD_DIR)))
    FINETUNED_CHECKPOINT = find_checkpoint_dir(downloaded_root)

print("Fine-tuned checkpoint:", FINETUNED_CHECKPOINT)
print("Checkpoint files:", sorted(path.name for path in FINETUNED_CHECKPOINT.iterdir()))
print("Working disk after checkpoint:", disk_usage_line(Path("/kaggle/working")))

# %% [markdown] {"jupyter":{"outputs_hidden":false}}
# ## 5. Download OmniVoice evaluation models
# OmniVoice objective metrics use separate pretrained evaluator models. SIM-o
# needs a WavLM-based ECAPA-TDNN speaker verification model, UTMOS needs its MOS
# prediction model, and WER for Filipino is computed with the Whisper-large-v3
# evaluator path used by OmniVoice for non-Chinese multilingual benchmarks.
...
# %% [markdown] {"jupyter":{"outputs_hidden":false}}
# ## 6. Build the PLD smoke-test evaluation manifest
# The pre-tokenized train/dev shards are not used here because objective TTS
# evaluation is performed on generated waveforms. This manifest starts from
# `data_clean/test.jsonl`, resolves the raw test WAV paths, and adds the fields
# expected by OmniVoice inference and evaluation scripts.
# For speaker similarity, the prompt should be a real reference utterance from
# the same speaker. We therefore pair each test target with a different test
# utterance from the same PLD speaker when available.
...
# %% [markdown] {"jupyter":{"outputs_hidden":false}}
# ## 7. Run base and fine-tuned inference
# Both models synthesize the same smoke-test manifest. The base model is loaded
# from `k2-fsa/OmniVoice`, while the fine-tuned model is loaded from the W&B
# checkpoint artifact. Keeping the manifest and inference parameters identical
# makes the metric comparison attributable to the model weights rather than the
# evaluation setup.
...
generated_dirs = {
    "base": run_inference_set(PRETRAINED_MODEL, "base", BASE_RESULTS_DIR),
...
# WER measures intelligibility: generated audio is transcribed by an ASR model
# and compared with the target text. Lower WER is better.
# SIM-o measures speaker similarity: a speaker encoder embeds the reference
# prompt and generated audio, then reports cosine similarity. Higher SIM-o is
# better.
# UTMOS estimates naturalness with a learned MOS predictor. It is not a
# replacement for human listening tests, but it is useful for tracking whether a
# model produces audio that sounds more speech-like. Higher UTMOS is better.
...
# The OmniVoice metric scripts write detailed text logs. This section extracts
# the headline values into a compact JSON summary and W&B table so base and
# fine-tuned results can be compared directly in the W&B run dashboard.
...
if wandb_run is not None:
    ...
    wandb_run.log({"evaluation_metric_summary": metrics_table})

# %% [markdown] {"jupyter":{"outputs_hidden":false}}
# ## 10. Log evaluation artifacts to W&B
# The final W&B artifact stores the evaluation manifest, raw metric logs, and
# parsed summary. A small audio table is also logged for listening checks, with
# the five selected test WAVs prioritized so they are easy to find alongside the
# smoke-test aggregate metrics.
...
def log_wandb_outputs() -> None:
    ...
    results_artifact = wandb.Artifact(
        "omnivoice-filipino-smoke-test-eval-results",
        type="evaluation",
        metadata={
            "run_name": RUN_NAME,
    ...
    if LOG_AUDIO_EXAMPLES_TO_WANDB:
        ...
        wandb_run.log({"evaluation_audio_examples": audio_table})
