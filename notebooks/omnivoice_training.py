...
# # OmniVoice Filipino PLD full fine-tuning pipeline on Kaggle
# This notebook-style script runs the current full training workflow:
...
# - log training curves, configs, checkpoint artifact, and playable WAV samples to W&B.
...
OUTPUT_DIR = WORK_DIR / "exp_full"
RESULTS_DIR = WORK_DIR / "results" / "full"
BASELINE_RESULTS_DIR = RESULTS_DIR / "base"
FINETUNED_RESULTS_DIR = RESULTS_DIR / "finetuned"
METRICS_DIR = WORK_DIR / "metrics" / "full"
...
WANDB_MODE = "online"
WANDB_TAGS = ["omnivoice", "filipino", "pld", "kaggle", "full"]
...
def wandb_env() -> dict[str, str]:
    ...
    env["OMNIVOICE_SAVE_ACCELERATOR_STATE"] = (
        "true" if SAVE_ACCELERATOR_STATE else "false"
    )
...
# %% [markdown] {"jupyter":{"outputs_hidden":false}}
# ## 3. Install OmniVoice and runtime patches
...
def patch_omnivoice_tracker_support(repo_dir: Path) -> None:
    trainer_path = repo_dir / "omnivoice" / "training" / "trainer.py"
    text = trainer_path.read_text(encoding="utf-8")

    if "OMNIVOICE_ACCELERATE_LOG_WITH" not in text:
        old = """        accelerator = Accelerator(
...
"""
        new = """        tracker_names = [
            ...
            for name in os.environ.get(
                "OMNIVOICE_ACCELERATE_LOG_WITH", "tensorboard"
            ).split(",")
            if name.strip()
        ]
...
"""
        if old not in text:
            raise RuntimeError("Could not find the expected Accelerator block.")
        text = text.replace(old, new)

    old_init = '        accelerator.init_trackers("tensorboard")'
    new_init = (
    ...
    print("Patched OmniVoice tracker support:", trainer_path)


def patch_omnivoice_checkpoint_saving(repo_dir: Path) -> None:
    checkpoint_path = repo_dir / "omnivoice" / "training" / "checkpoint.py"
    text = checkpoint_path.read_text(encoding="utf-8")

    old = """    # 1. Save Accelerator State (Optimizer, Scheduler, RNG, Scaler)
    accelerator.save_state(checkpoint_dir)
"""
    new = """    # 1. Optionally save Accelerator State (Optimizer, Scheduler, RNG, Scaler).
    save_accelerator_state = os.environ.get(
        "OMNIVOICE_SAVE_ACCELERATOR_STATE", "false"
    ...
    if save_accelerator_state:
        ...
        logger.info(
            "Skipping accelerator state save. "
            "Set OMNIVOICE_SAVE_ACCELERATOR_STATE=true to save optimizer state."
        )
    accelerator.wait_for_everyone()
"""
    if old in text and "OMNIVOICE_SAVE_ACCELERATOR_STATE" not in text:
        text = text.replace(old, new)
        checkpoint_path.write_text(text, encoding="utf-8")
        print("Patched OmniVoice checkpoint saving:", checkpoint_path)
    elif "OMNIVOICE_SAVE_ACCELERATOR_STATE" in text:
        print("OmniVoice checkpoint saving already patched:", checkpoint_path)
    else:
        raise RuntimeError("Could not find the expected accelerator.save_state block.")


patch_omnivoice_tracker_support(OMNIVOICE_DIR)
patch_omnivoice_checkpoint_saving(OMNIVOICE_DIR)
...
def log_wandb_artifacts() -> None:
    ...
    if LOG_CHECKPOINT_ARTIFACT_TO_WANDB and LATEST_CHECKPOINT is not None:
        checkpoint_artifact = wandb.Artifact(
            f"omnivoice-filipino-full-{LATEST_CHECKPOINT.name}",
            type="model",
            metadata={
                "run_name": RUN_NAME,
                ...
                "save_accelerator_state": SAVE_ACCELERATOR_STATE,
            },
        )
