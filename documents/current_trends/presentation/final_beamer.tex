...
\begin{frame}{Fine-Tuning Setup}
  ...
  \begin{table}
    ...
    \begin{tabular}{llll}
      ...
      \midrule
      FT-5e-6 & 5e-6 & 5000 & lowest dev loss \\
      FT-1e-5 & 1e-5 & 5000 & lowest dev loss (found at step 4900) \\
      FT-2e-5 & 2e-5 & 5000 & lowest dev loss \\
      \bottomrule
    \end{tabular}
  \end{table}

  \vspace{0.4em}
  \small A \textbf{controlled comparison}: same steps, same checkpoint policy, only the learning rate differs. (Two earlier exploratory runs at 1000 and 2000 steps with final-step checkpoints are project history.)
\end{frame}
...
\begin{frame}{Training Behavior}
  ...
  \vspace{0.3em}
  \small W\&B training-loss curves for the fine-tuning runs, including the preliminary exploratory runs.
\end{frame}
