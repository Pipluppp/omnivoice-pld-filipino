...
Two preliminary runs (1000 steps at learning rate 2e-5 and 2000 steps at 5e-6, both keeping the final-step checkpoint) first established that the workflow runs end to end on Kaggle T4 hardware. Because they mixed checkpoint policy, step count, and learning rate, their results could not isolate any single setting and are treated as project history. The final comparison uses three controlled runs that share the same 5000-step budget and the same development-loss-selected checkpoint policy --- the development set is evaluated at fixed intervals and a checkpoint is saved only when development loss improves --- and differ only in learning rate:

\begin{itemize}
  \item \textbf{FT-5e-6:} the most conservative update; best-development-loss checkpoint.
  ...
  \item \textbf{FT-2e-5:} the most aggressive update; best-development-loss checkpoint.
\end{itemize}
...
\begin{table}[t]
  ...
  \begin{tabular}{lrrrr}
    ...
    FT-5e-6 & 21.96 & $-$0.59 & \textbf{0.605} & 3.60 \\
    FT-1e-5 (step 4900) & \textbf{18.52} & $-$4.03 & 0.604 & 3.61 \\
    FT-2e-5 & 18.83 & $-$3.72 & 0.583 & 3.61 \\
    \bottomrule
  \end{tabular}
\end{table}
...
The earlier exploratory runs reinforce this. The preliminary 1000-step 2e-5 and 2000-step 5e-6 runs, which kept final-step checkpoints, produced 20.07\% and 22.64\% WER respectively --- the second slightly worse than the base model. Under the controlled 5000-step best-development-loss setup, both learning rates improved on their exploratory counterparts, showing that the longer budget and development-loss-selected checkpointing contributed real gains independent of learning rate.
