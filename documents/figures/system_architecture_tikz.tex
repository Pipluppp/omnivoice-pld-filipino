...
\begin{scope}[on background layer]
  \filldraw[draw=archdataedge!70, fill=archdatafill!40, dashed, rounded corners=8pt, line width=0.7pt]
    (0.7, -0.8) rectangle (15.5, -2.8);
  \node[phaseTitle, text=archdataedge, anchor=north west] at (0.8, -0.9) {1. Dataset preparation};

  \filldraw[draw=archtrainedge!70, fill=archtrainfill!40, dashed, rounded corners=8pt, line width=0.7pt]
    (0.7, -3.0) rectangle (19.3, -6.3);
  \node[phaseTitle, text=archtrainedge, anchor=north west] at (0.8, -3.1) {2. Tokenization and supervised fine-tuning};

  \filldraw[draw=archevaledge!70, fill=archevalfill!40, dashed, rounded corners=8pt, line width=0.7pt]
    (0.7, -6.7) rectangle (19.3, -10.7);
  \node[phaseTitle, text=archevaledge, anchor=north west] at (0.8, -6.8) {3. Held-out evaluation and delivery};
\end{scope}
...
\node[trainBox] (tokenizer) at (2.8, -5.0) {\textbf{Higgs Audio v2}\\continuous waveform\\to discrete tokens};

\coordinate (gridOrigin) at (6.25, -5.56);
\coordinate (textOrigin) at ($(gridOrigin)+(0.4, 1.35)$);

\node[font=\sffamily\scriptsize\bfseries, text=archink] (gridTitle) at (8.15, -3.6) {8-codebook acoustic tokens};
...
\node[note, anchor=west] at ($(textOrigin)+(3.15, 0.14)$) {text};
...
\node[trainBox] (omni) at (13.5, -5.0) {\textbf{OmniVoice}\\pretrained multilingual TTS\\masked diffusion LM};

\node[smallBox, fill=archtrainfill, draw=archtrainedge] (sft)  at (17.5, -4.35) {\textbf{SFT objective}\\weighted cross-entropy};
\node[smallBox, fill=archtrainfill, draw=archtrainedge] (ckpt) at (17.5, -5.65) {\textbf{Best-dev}\\checkpoint selection\\LR comparison};

\draw[arrow] (split.south) -- ++(0,-0.4) -| (gridTitle.north);
...
\node[evalBox] (infer) at (2.8, -9.0) {\textbf{Zero-shot inference}\\matched test prompts\\+ reference voices};
\node[evalBox] (gen)   at (6.4, -9.0) {\textbf{Generated}\\Filipino speech};

\node[evalBox] (wer) at (10.0, -8.0) {\textbf{Whisper ASR}\\WER};
\node[evalBox] (sim) at (10.0, -9.0) {\textbf{ECAPA-TDNN}\\SIM-o};
\node[evalBox] (mos) at (10.0, -10.0) {\textbf{UTMOS}\\naturalness};

\node[deployBox] (react) at (13.5, -9.0) {\textbf{React audio}\\showcase};

\node[deployBox] (cf) at (17.5, -9.0) {
...
\draw[dashArrow] (ckpt.south) -- (17.5, -8.85);
\node[note, anchor=west] at (17.6, -7.5) {selected checkpoint};
