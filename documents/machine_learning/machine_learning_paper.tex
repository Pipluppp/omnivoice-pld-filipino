...
Two preliminary runs first established the workflow: a 1000-step run at \texttt{2e-5} and a 2000-step run at \texttt{5e-6}, both keeping the final-step checkpoint. Because those runs mixed checkpoint policy, step count, and learning rate, their results could not isolate the effect of any single setting. The final comparison therefore uses three controlled runs that share the same 5000-step budget and the same checkpoint policy and differ only in learning rate: \texttt{5e-6}, \texttt{1e-5}, and \texttt{2e-5}. With the 1\% warmup, each controlled run uses 50 warmup steps.

All three controlled runs evaluated the development set at fixed intervals and saved a checkpoint only when development loss improved, so each run contributes its best-development-loss checkpoint to the comparison. In the \texttt{1e-5} run the lowest development loss, 4.162, occurred at step 4900, so checkpoint-4900 became that run's selected model. Differences between checkpoints are small in loss terms, which is exactly why downstream audio metrics, not loss alone, decide the final model in Section~\ref{sec:results}.

Weights and Biases is used for experiment tracking \citep{wandb}. Each training run logs configuration values, training loss, development loss, checkpoints, generated samples, and evaluation artifacts. Hyperparameter tuning progressed from the preliminary exploratory runs to the controlled learning-rate comparison, with every run's selected checkpoint stored as a W\&B artifact. The consolidated W\&B report for this project is available as the \href{https://wandb.ai/duncanb013-polytechnic-university-of-the-philippines/OmniVoice-PLD-Filipino/reports/OmniVoice-PLD-Filipino-finetuning--VmlldzoxNjk0ODkyMg?accessToken=ktcztpc3ocu4gmtctis13mdubb82ec1r4rnyuxdta56tts4z5j4f965olbm0v5ka}{OmniVoice PLD Filipino fine-tuning report}. The logged runs make it possible to compare whether lower development loss corresponds to better WER, SIM-o, UTMOS, and listening quality. Selected training curves, run screenshots, and generated-audio sample tables are reproduced in Appendices~\ref{app:ml-wandb-curves}--\ref{app:ml-sample-tables}.
...
\begin{table}[H]
  ...
  \begin{tabular}{p{0.27\linewidth}p{0.27\linewidth}rrrr}
    ...
    Fine-tuned, lr 5e-6 & best dev loss & 21.96 & -0.59 & \textbf{0.605} & 3.60 \\
    Fine-tuned, lr 1e-5 & best dev loss at step 4900 & \textbf{18.52} & -4.03 & 0.604 & 3.61 \\
    Fine-tuned, lr 2e-5 & best dev loss & 18.83 & -3.72 & 0.583 & 3.61 \\
    \bottomrule
  \end{tabular}
...
Earlier exploratory runs reinforce this. A 1000-step run at 2e-5 and a 2000-step run at 5e-6, both keeping the final-step checkpoint, produced 20.07\% and 22.64\% WER respectively, the second slightly worse than the base model. Those runs mixed checkpoint policy, step count, and learning rate, which is precisely why the final comparison holds the first two constant. Under the controlled setup, both the 2e-5 and 5e-6 learning rates improved on their exploratory counterparts, showing that the 5000-step budget and best-development-loss selection contributed real gains independent of learning rate.
...
\begin{figure}[H]
  \centering
  \caption{Combined W\&B training-loss curves for the Filipino OmniVoice fine-tuning runs, including the preliminary exploratory runs.}
...
\begin{figure}[H]
  \centering
  \caption{W\&B run screenshot for the preliminary exploratory 1000-step run at learning rate 2e-5 (project history; not part of the controlled comparison).}
...
\begin{figure}[H]
  \centering
  \caption{W\&B run screenshot for the preliminary exploratory 2000-step run at learning rate 5e-6 (project history; not part of the controlled comparison).}
...
\begin{figure}[H]
  \centering
  \caption{W\&B generated-test-samples table for the preliminary exploratory 1000-step checkpoint (project history).}
...
\begin{figure}[H]
  \centering
  \caption{W\&B generated-test-samples table for the preliminary exploratory 2000-step checkpoint (project history).}
